...
class GS_AttackProblem(ea.Problem):
    ...
    def aimFunc(self, pop):
        ...
        # if self.num % 50 == 0:  # haha  画特征图
        #     mean_activation_map = feature_map.squeeze(0).mean(dim=0).cpu().detach().numpy()
        #     plt.imshow(mean_activation_map, cmap='viridis')
        #     plt.show()
        # elif self.num % 5 == 0:
        #     mean_activation_map = feature_map.squeeze(0).mean(dim=0).cpu().detach().numpy()
        #     self.feature_maps.append(mean_activation_map)
    ...
    def split_and_optimize(self):
        ...
        for split_idx in range(len(sub_gaussians_list)):
            ...
            algorithm = ea.moea_NSGA2_templet(
                problem,
                ea.Population(Encoding='RI', NIND=50),  # haha
                MAXGEN=200,  # 每个分割部分的最大进化代数 haha
                logTras=0
            )
            ...
            # 【【【新增这行代码】】】
            # 将包含 Vars 和 ObjV 的完整结果保存到 npz 文件中
            np.savez(
                f"results_split_{split_idx}.npz",
                Vars=res['Vars'],  # 保存帕累托前沿解的基因
                ObjV=res['ObjV']  # 保存帕累托前沿解的目标函数值
            )
...
if __name__ == "__main__":
    ...
    save_dir = 'output/room_attack/merged_gaussians_' # haha
