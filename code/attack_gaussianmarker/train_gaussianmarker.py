...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if gt_image.shape != image.shape:
            # 使用 Resize 调整 gt_image 的大小
            print("修改了尺寸！！", image.shape, gt_image.shape)
            resize_transform = transforms.Resize((image.shape[1], image.shape[2]))
            gt_image = resize_transform(gt_image.unsqueeze(0)).squeeze(0)
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                point_cloud_path = os.path.join(os.path.dirname(scene_model_path), "output_wm",
                                                os.path.basename(scene_model_path), "point_cloud",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[2000, 10000, 20000])  # 200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800, 2000
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[2000, 10000, 20000])
