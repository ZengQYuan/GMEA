...
class GS_AttackProblem(ea.Problem):
    ...
    def aimFunc(self, pop):
        ...
        for i in range(num_individuals):
            ...
            with torch.no_grad():
                temp_render = render(self.pipe["viewpoint"],
                                     temp_gaussians,
                                     self.pipe["pipe"],
...
class SplitGS_AttackProblem:
    def __init__(self, original_gaussians, target_image, pipe_params, decoder, num_splits):
    ...
    def split_and_optimize(self):
        ...
        def generate_sub_target_images(sub_gaussians_list):
            ...
            for sub_gaussians in sub_gaussians_list:
                # 渲染当前子模型并生成目标图像
                with torch.no_grad():
                    sub_render = render(self.pipe["viewpoint"],
                                        sub_gaussians,
                                        self.pipe["pipe"],
                                        self.pipe["background"])
                    target_images.append(sub_render["render"])
            return target_images


        all_results = {}
        sub_gaussians_list = split_gaussians_by_kmeans(self.original_gaussians, k=self.num_splits)
        sub_target_images = generate_sub_target_images(sub_gaussians_list)



        for split_idx in range(len(sub_gaussians_list)):
            problem = GS_AttackProblem(sub_gaussians_list[split_idx], sub_target_images[split_idx], self.pipe, self.decoder)
...
if __name__ == "__main__":
    ...
    split_problem = SplitGS_AttackProblem(
        original_gaussians,
        target_image,
        {'viewpoint': viewpoint_cam, 'pipe': pipe_params, 'background': background},
        decoder,
        num_splits=4
    )
    ...
    # 渲染合并后的结果
    with torch.no_grad():
        merged_render = render(viewpoint_cam, merged_gaussians, pipe_params, background)
    ...
    # 保存结果
    save_dir = "output/chair_attack/merged_gaussians"
