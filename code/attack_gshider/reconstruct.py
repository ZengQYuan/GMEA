...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    if checkpoint:
        ...
        print(f"\n[INFO] Resuming training from checkpoint: {checkpoint}")

        # Load the main Gaussian Model
        gaussians = load_ply_as_initialization(ply_path, dataset, opt)

        # Construct paths for the other network models
        net_checkpoint_path = checkpoint.replace('.pth', '_net.pth')
        waternet_checkpoint_path = checkpoint.replace('.pth', '_waternet.pth')

        # Load imagenet weights if the file exists
        if os.path.exists(net_checkpoint_path):
            print(f"[INFO] Loading ImageNet from: {net_checkpoint_path}")
            imagenet.load_state_dict(torch.load(net_checkpoint_path, weights_only=False))
        else:
            print(f"[WARNING] ImageNet checkpoint not found at: {net_checkpoint_path}")

        # Load waternet weights if the file exists
        if os.path.exists(waternet_checkpoint_path):
            print(f"[INFO] Loading WatermarkNet from: {waternet_checkpoint_path}")
            waternet.load_state_dict(torch.load(waternet_checkpoint_path, weights_only=False))
        else:
            print(f"[WARNING] WatermarkNet checkpoint not found at: {waternet_checkpoint_path}")

        print(f"[INFO] Resuming from iteration {first_iter}\n")
        # --- End of modification ---

    print("\n[INFO] Freezing decoder networks. Only the Gaussian model will be trained.")
    for param in imagenet.parameters():
        param.requires_grad = False
    for param in waternet.parameters():
        param.requires_grad = False
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        L1 = (1.0 - opt.lambda_dssim) * l1_loss(rgb_image, gt_image) + opt.lambda_dssim * (
                    1.0 - ssim(rgb_image, gt_image))
        ...
        if iteration % 2000 == 0:
            plt.imshow(rgb_image.detach().cpu().numpy().transpose((1, 2, 0)))
            plt.show()
        if iteration % 1999 == 0:
            plt.imshow(water_image.detach().cpu().numpy().transpose((1, 2, 0)))
            plt.show()
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, imagenet, waternet,
    ...
    if iteration in testing_iterations:  # if iteration in testing_iterations or time_flag:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                print(
                    "\n[ITER {}]  Evaluating {}: L1 {} PSNR {}  SSIM {}  LPIPS  {} PSNR_Water {} SSIM_Water {}".format(
                        iteration, config['name'], l1_test, psnr_test, ssim_test, lpips_test, psnr_water, ssim_water))
                if tb_writer:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int,
                        default=[3_000, 7_000, 10_000, 15_000, 20_000, 25_000, 30_000, 40_000, 50_000])
    parser.add_argument("--save_iterations", nargs="+", type=int,
                        default=[10_000, 15000, 20000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int,
                        default=[3_000, 7_000, 10_000, 20_000, 30_000, 40_000, 50_000])
    parser.add_argument("--start_checkpoint", type=str, default="output/orchids_wm/chkpnt10000.pth")  # None, "output/garden/chkpnt10000.pth" 'output/fortress_attack/merged_gaussians/point_cloud/iteration_2000/point_cloud.ply'
    args = parser.parse_args(sys.argv[1:])
    args.save_iterations.append(args.iterations)

    ply_path = 'output/orchids_attack/merged_gaussians/point_cloud/iteration_2000/point_cloud.ply'
    save_path = "output/orchids_reconstruct/point_cloud/iteration_"+str(op.extract(args).iterations)+"/point_cloud.ply"
